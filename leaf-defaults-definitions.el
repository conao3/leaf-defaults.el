;;; leaf-defaults-definitions.el --- Config collection for :defaults keyword  -*- lexical-binding: t; -*-
...
;;   (require 'leaf-defaults-definitions)
;;   (leaf-defaults-definitions--leaf/leaf-keyword)

;;   ;; load prepared configures you want.
;;   (leaf-defaults-definitions--leaf-defaults/base)
;;   (leaf-defaults-definitions--leaf/ivy)
...
(prog1 'emacs
  ...
  (leaf *leaf-keywords-config
    :preface
    (defun leaf-defaults-definitions--manager nil
      ...
      (let ((leaf-manager-recursive-edit t)
            (leaf-manager-file (eval-when-compile
                                 (or load-file-name
                                     ...
                                     (buffer-file-name))))
            (leaf-manager-template-feature-name "leaf-defaults-definitions")
  ...
  (defun leaf-defaults-definitions-basic-dependency nil
    "Install leaf-defaults dependency to use prepared configures."
    (leaf blackout
      :ensure t))

  (eval-and-compile
    (defmacro leaf-defaults-definitions--generate (&rest plist)
      ...
      (let ((plist* (leaf-normalize-plist plist 'merge))
        ...
        (while plist*
          (let* ((key (leaf-sym-from-keyword
            ...
            (push
             `(defun ,(intern
                       (format "leaf-defaults-definitions--leaf-defaults/%s" key)) nil
                ,(format "Autogenerated bundle leaf-defaults-definitions for %s." key)
                ,@(mapcan
                   (lambda (elm)
                     (let ((fn (intern
                                (format "leaf-defaults-definitions--leaf/%s" elm))))
                       `((declare-function ,fn "leaf-defaults-definitions")
                         (,fn))))
                   val))
  ...
  (leaf-defaults-definitions--generate
...
(provide 'leaf-defaults-definitions)
...
;;; leaf-defaults-definitions.el ends here
